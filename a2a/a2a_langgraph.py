...
- 아키텍처
    - Langgraph
    - State : 대화 내용, 수정 횟수를 저장하는 공유 메모리
    - Node
        - Coder Node    : 상태(메모리)를 읽고 코드 신규 생성/수정
        - Reviewer Node : 코드를 검증하고 Pass / Fail 판정(리뷰 내용 전달)
    - Edge
        - entry_point       : Coder node
        - code_             : Code node -> Reviewer Node
        - Conditional Edge  : Reviewer의 판정에 따라 Coder Node 갈지, End 갈지 결정
'''
...
# 2. 상태 정의 (메세지(messages), 재시도 횟수(iterations), AgentState)
class AgentState(TypedDict):
    # Annotated[ ... ] : 메타데이터 표기
    # 타입힌트 제시, operator.add 전달 -> 사앹 업데이트 방식 명확하게 지정
    # 덮어쓰지 말고, 기존 내용에 새로운 내용을 추가하라
    # 대화를 기억하는 방법 -> 누적하여 보관
    messages : Annotated[ List[BaseMessage], operator.add ]
    # 재시도 횟수, 리뷰등을 통해 수정 내용 발생시 최대 순환 수정 회수 제한 비교용
    iterations : int
...
def coder_node( state:AgentState ):
    ...
    coder_prompt = ChatPromptTemplate.from_messages([
        ...
        # 해당 메세지가,  HumanMessage, AIMessage, SystemMessage or 형태를 잘 모르겠다 => 알아서 세팅 placeholder
        # user => placeHolder로 변경
...
def reviewer_node(state:AgentState):
    ...
    reviewer_prompt = ChatPromptTemplate.from_messages([
        ...
        "보안 취약점, 비효율적인 부분, 스타일 가이드 등을 점검하고 수정 제안을 하세요.\n"
...
def is_continue(state:AgentState):
    ...
    # 1. 안전장치, 무제한으로 풀면 => 토큰을 무제한 사용 => 비용 폭탄
    if iterations >= 3: # 최대 3번만 반복
        print('-- [System] 최대 반복횟수 도달, 추론 행위를 종료합니다. --')
        return 'my_end'
    # 2. 리뷰 통과 여부 체크
    if "PASS" in last_msg: # 메세지 중에 존재만 하면 종료
...
if __name__ == '__main__':
    # 초기 질문
    # 반복 행위를 위해서 비효율적인 주문 제시
    initial_input = '리스트에서 중복을 제거하고, 정렬하는 파이썬 함수를만들어줘. 단, 좀 비효율적으로 작성해줘'
    # 초기 상태값 구성
    inputs = {
        'message': [HumanMessage(content=initial_input)],
        'iterations' : 0
    }
    print('시작요청', inputs)
