...
class BedrockMCPAgent:
    ...
    ##  _${fucntion_name}과 같이 하면 내부 함수
    ##  __${function_name)으로 명시하면 private로 지정됨.
    # llm 생성
    ## 이하, a2a/a2a_langgraph 3번 항목 참조함.
    async def _init_llm(self):
        try:
            ...
            # d특정 회사 라이브러리 설치하면 가능
            # pip install anthropic openai ....
            # from langchain_anthropic import ChatAnthropic
            # 다른 모델 사용, 특정 회사 모델 직접 사용(클로드, gpt, 제미나이 등)
            print('Bedrock LLM 객체 생성 실패')
            pass
        # 그래프 구성
        # 사용자 요청 처리(프럼프트 처리)
        # 메모리 정리(뒷정리)
    ...
    def _setup_graph( self ):
        ...
        # 함수 내부에 함수가 존재 => 내부함수, 함수 내부에서만 사용 가능, 외부 사용 X
        def call_agent(state:MessagesState) -> dict:
            '''LLM을 호출하여 Tool 선택, 응답 생성'''
            pass
        ...
        # 4. 엣지 (규칙, 순서, 시작, 종료 등) 설정(정의)
        # 시작점
        workflow.set_entry_point('agent') # llm 일차 판단
        def 조건부함수()->str:
            '''tool_calls 값 체크하여, 툴로 진행할지 끝낼지 판단.'''
